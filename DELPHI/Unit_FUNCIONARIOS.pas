...
type
  TForm_FUNCIONARIOS = class(TForm)
    ...
    ADOQuery1CIDADE_FUNCIONÁRIO: TIntegerField;
...
Begin
   ...
   DBLookupComboBox1.KeyValue := ADOQuery1CIDADE_FUNCIONÁRIO.AsInteger;
...
begin
  ...
  if cont < 10 then
  begin
     Application.MessageBox('Preencha todos os dígitos da data!', 'Atenção!', MB_ICONWARNING);
     MaskEdit1.SetFocus;
  end;
end;
...
begin
  ...
  if cont < 11 then
  begin
     Application.MessageBox('Preencha todos os dígitos do CPF!', 'Atenção!', MB_ICONWARNING);
     MaskEdit3.SetFocus;
  end;
...
begin
  ...
  hora_c := Copy(lbl_data.Caption,14,8);
    ...
    if Application.MessageBox('Tem certeza que deseja alterar o funcionário',
   ...
   Cod_Funcionario_Alterado := DBGrid1.DataSource.DataSet.RecNo  ;
    ...
    with GRAVA_LOG do
    begin
      Parameters.ParamByName('@DESCRICAO').Value := 'Funcionário nº ' + Edit1.Text + ' alterado.' ;
