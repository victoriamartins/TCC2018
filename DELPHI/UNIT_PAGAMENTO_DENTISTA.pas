...
begin
    ...
    if RadioGroup2.ItemIndex = -1 then //só o MOSTRAR
    begin
    ...
    begin
        ...
        ALTERA_PAGAMENTO_DENTISTA.ExecProc;
            ...
            ADOQuery1.Active := true ;
                    ...
                    with GRAVA_LOG do
                    begin
                      Parameters.ParamByName('@DESCRICAO').Value := 'Pagamento nº ' + Label3.Caption + ' pago.' ;
...
begin
    ...
    begin
        Application.MessageBox('Não é possível alterar um pagamento pago!', 'Atenção!', MB_ICONWARNING);
    end
...
begin
  ...
  begin
    Application.MessageBox('Preencha o campo de data!', 'Atenção!', MB_ICONWARNING);
    MaskEdit1.SetFocus;
    Exit;
  end;
  if (MaskEdit1.Text = '0,00')or(MaskEdit1.Text = EmptyStr)then
  begin
    Application.MessageBox('Preencha o campo de valor!', 'Atenção!', MB_ICONWARNING);
    Edit2.SetFocus;
    Exit;
  end;
 ...
 if Application.MessageBox('Tem certeza que deseja cadastrar a consulta?','CADASTRAR', mb_iconquestion + mb_yesno) = idYes then
  begin
       ...
       hora_c := Copy(lbl_data.Caption,14,8);
        ...
        with GRAVA_LOG do
        begin
          Parameters.ParamByName('@DESCRICAO').Value := 'Pagamento nº ' + ADOQuery1COD_PAGAMENTO.AsString + ' alterado.' ;
          Parameters.ParamByName('@USUARIO').Value := Unit_TELA_PRINCIPAL.Form_TELA_PRINCIPAL.QUERY_USUARIOCOD_USUARIO.AsInteger;
          Parameters.ParamByName('@DTHORA').Value := ano_c + '-' + mes_c + '-' + dia_c + ' ' + hora_c;
          ExecProc;
        end;
