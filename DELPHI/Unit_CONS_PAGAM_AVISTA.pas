...
begin
    ...
    if ADOQuery1TIPO_AVISTA.AsString = 'CARTÃO'  then
    begin
        Label15.Caption := 'CARTÃO';
...
begin
    ...
    begin
         ...
         begin
            ...
            ADOQuery1.sql.Text := 'SELECT COD_AVISTA, NOME_PACIENTE,PACIENTE_AVISTA,NOME_DENTISTA,DENTISTA_AVISTA,'+
                                  ...
                                  ' FROM TB_PAGAMENTO_PACIENTE_AVISTA,TB_PACIENTE,TB_DENTISTA   '+
                                  '	WHERE PACIENTE_AVISTA=COD_PACIENTE AND DENTISTA_AVISTA=COD_DENTISTA '+
                                  ' AND TIPO_AVISTA = '+ QuotedStr('CARTÃO');
            ADOQuery1.Active := true;
            ADOQuery1.open;
         end
    ...
    begin
        ...
        begin
               ...
               else if RadioGroup2.ItemIndex = 1 then //cartã
               begin
                  ADOQuery1.close;
                  ADOQuery1.Active := false;
                  ADOQuery1.SQL.Clear;
        ...
        begin
               ...
               else if RadioGroup2.ItemIndex = 1 then //cartão
               begin
                  ADOQuery1.close;
                  ADOQuery1.Active := false;
                  ADOQuery1.SQL.Clear;
      ...
      begin
        ...
        with CAIXA_AUTOMATICO do
         begin
          ...
          with GRAVA_LOG do
          begin
            Parameters.ParamByName('@DESCRICAO').Value := 'Pagamento à vista nº ' + ADOQuery1COD_AVISTA.AsString + ' pago.' ;
    ...
    begin
          ...
          begin
                      ...
                      begin
                          ...
                          ' VALOR_AVISTA,STATUS_PAGAMENTO,DATAPAGAMENTO_AVISTA, TIPO_AVISTA,  STATUS_PAGAMENTO,DTHORACADASTRO_AVISTA '+
                          ' FROM TB_PAGAMENTO_PACIENTE_AVISTA,TB_PACIENTE,TB_DENTISTA   '+
                          '	WHERE PACIENTE_AVISTA=COD_PACIENTE AND DENTISTA_AVISTA=COD_DENTISTA AND TIPO_AVISTA= '+ QuotedStr('CARTÃO')+
                          ' AND NOME_PACIENTE LIKE '+QuotedStr(Edit6.Text+'%');
                          ADOQuery1.Active := true;
                          ADOQuery1.open;
                      end
...
begin
    ...
    begin
      Application.MessageBox('Não é possivel alterar! Selecione um registro!','ATENÇÃO!',MB_ICONEXCLAMATION);
      exit;
    end;
...
begin
    ...
    begin
        Application.MessageBox('Selecione um dentista na tabela de dentistas!', 'Atenção!', MB_ICONWARNING);
        Edit3.SetFocus;
        Exit;
    end
    ...
    begin
        Application.MessageBox('Selecione um paciente na tabela de pacientes!', 'Atenção!', MB_ICONWARNING);
        Edit2.SetFocus;
        Exit;
    end
    ...
    begin
        Application.MessageBox('Preencha o campo da data!', 'Atenção!', MB_ICONWARNING);
        MaskEdit1.SetFocus;
        Exit;
    end
    ...
    begin
        Application.MessageBox('Selecione um tipo de pagamento!', 'Atenção!', MB_ICONWARNING);
        Exit;
    end
    ...
    if Application.MessageBox('Deseja mesmo alterar o pagamento?','ATENÇÃO',
                                          MB_YESNO+MB_ICONEXCLAMATION)= IDYES then
    BEGIN
     ...
     END;
       ...
       begin
       ADOStoredProc1.Parameters.ParamByName('@TIPO').Value := 'CARTÃO';
       end
       else if ComboBox1.ItemIndex = 1 then
       begin
      ...
      hora_c := Copy(lbl_data.Caption,14,8);
        ...
        with GRAVA_LOG do
        begin
          Parameters.ParamByName('@DESCRICAO').Value := 'Pagamento à vista nº ' + Edit1.Text + ' alterado.' ;
          Parameters.ParamByName('@USUARIO').Value := Unit_TELA_PRINCIPAL.Form_TELA_PRINCIPAL.QUERY_USUARIOCOD_USUARIO.AsInteger;
          Parameters.ParamByName('@DTHORA').Value := ano_c + '-' + mes_c + '-' + dia_c + ' ' + hora_c;
          ExecProc;
        end;
