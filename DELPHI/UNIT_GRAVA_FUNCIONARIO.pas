...
begin
  ...
  if Application.MessageBox('Tem certeza que deseja cadastrar o funcionário?','CADASTRAR', mb_iconquestion + mb_yesno) = idYes then
  begin
    ...
    with GRAVA_LOG do
    begin
      Parameters.ParamByName('@DESCRICAO').Value := 'Funcionário ' + Edit1.Text + ', nº ' + INTtOSTR(GRAVA_FUNCIONARIO.Parameters.ParamByName('@RETURN_VALUE').Value) + ' cadastrado.' ;
...
begin
  ...
  begin
    Application.MessageBox(
      'O campo de Rg deve estar completo!', 'Atenção!',
      MB_ICONWARNING);
      MaskEdit2.Clear;
      MaskEdit2.SetFocus;
    Exit;
...
begin
  Application.MessageBox('Aqui você poderá efetuar o cadastro de um(uma) funcionário(a)!', 'Atenção!', MB_ICONEXCLAMATION);
end;
