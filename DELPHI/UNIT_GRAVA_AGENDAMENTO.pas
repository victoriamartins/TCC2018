...
begin
  ...
  begin
    Application.MessageBox('Selecione um paciente!', 'Atenção!', MB_ICONWARNING);
    Exit;
  end;

  if VarIsNull(DBLOOKUPCOMBOBOX_DENTISTA.KeyValue=0) then
  begin
    Application.MessageBox('Selecione um dentista!', 'Atenção!', MB_ICONWARNING);
  ...
  begin
    Application.MessageBox('Selecione um tratamento!', 'Atenção!', MB_ICONWARNING);
  ...
  begin
    Application.MessageBox('Digite uma data!', 'Atenção!', MB_ICONWARNING);
    MaskEdit1.SetFocus;
    Exit;
  end;
  ...
  begin
    Application.MessageBox('Digite uma hora!', 'Atenção!', MB_ICONWARNING);
    MaskEdit2.SetFocus;
    Exit;
  end;
  ...
  begin
    Application.MessageBox('Já existe um agendamento para esse horário', 'ATENÇÃO!', MB_ICONERROR);
    MaskEdit2.Clear;
    MaskEdit2.SetFocus;
    exit;
  end;
  ...
  if Application.MessageBox('Tem certeza que deseja cadastrar o pré-agendamento?','CADASTRAR', mb_iconquestion + mb_yesno) = idYes then
  begin
    ...
    with GRAVA_LOG do
    begin
      Parameters.ParamByName('@DESCRICAO').Value := 'Agendamento nº ' + IntToStr(GRAVA_AGENDAMENTO.Parameters.ParamByName('@RETURN_VALUE').Value) + ' cadastrado';
