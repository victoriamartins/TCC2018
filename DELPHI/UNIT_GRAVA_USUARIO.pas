...
begin
  ...
  if Edit2.Text='' then
  begin
    Application.MessageBox('Preencha o campo de login!', 'Atenção!', MB_ICONWARNING);
    Edit2.SetFocus;
    Exit;
  end;
  ...
  if Edit3.Text='' then
  begin
    Application.MessageBox('Preencha o campo de senha!', 'Atenção!', MB_ICONWARNING);
    Edit3.SetFocus;
    Exit;
  end;
  ...
  begin
    Application.MessageBox('Selecione o tipo de usuário!', 'Atenção!', MB_ICONWARNING);
    Exit;
  end;

  if RadioGroup2.ItemIndex = -1 then
  begin
    Application.MessageBox('Selecione o status do usuário!', 'Atenção!', MB_ICONWARNING);
    Exit;
  end;
 ...
 GRAVA_USUARIO.Parameters.ParamByName('@DTHR_CADASTRO').Value := ano_c+'-'+ mes_c +'-'+ dia_c + ' ' + hora_c ;
  ...
  if Application.MessageBox('Tem certeza que deseja cadastrar o usuário?','CADASTRAR',
                              mb_iconquestion + mb_yesno) = idYes then
  begin
    GRAVA_USUARIO.ExecProc;
    Application.MessageBox('Usuário cadastrado!', 'AVISO', MB_OK);
    Image6Click(SELF);
  end
...
begin
  ...
  BEGIN
    Application.MessageBox('Esse login já está em uso!', 'Erro!', MB_ICONERROR);
    Edit2.Clear;
    exit ;
  END;
...
begin
  ...
  begin
    Application.MessageBox('As senhas não coincidem!', 'Atenção!', MB_ICONWARNING);
    Edit5.clear;
    Edit3.SetFocus;
    Exit;
  end;
...
begin
  ...
  begin
    Application.MessageBox('Selecione o dentista!', 'Atenção!', MB_ICONWARNING);
  ...
  begin
    Application.MessageBox('Selecione o funcionário!', 'Atenção!', MB_ICONWARNING);
  ...
  if Edit5.Text='' then
  begin
    Application.MessageBox('Preencha o campo de confirmação de senha!', 'Atenção!', MB_ICONWARNING);
    Edit5.SetFocus;
    Exit;
  end;
  ...
  if RadioGroup2.ItemIndex=-1 then
  begin
    Application.MessageBox('Escolha se o usuário é um dentista ou um funcionário!', 'Atenção!', MB_ICONWARNING);
    Exit;
  end;
  ...
  begin
    ...
    with GRAVA_LOG do
    begin
      Parameters.ParamByName('@DESCRICAO').Value := 'Usuário ' + Edit2.Text + ', código nº ' + INTtOSTR(GRAVA_USUARIO.Parameters.ParamByName('@RETURN_VALUE').Value) + ' cadastrado.' ;
